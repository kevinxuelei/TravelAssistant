...
- (NSString *)stringFromDate:(NSDate *)date {
    if (!dateFormattor) {
        ...
        [dateFormattor setDateFormat:@"yyyy 年 MM 月"];
    }
...
- (void)setupTitleBar {
    ...
    UIButton *titleButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, titleView.frame.size.width / 2, 44)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    self.scrollView.contentOffset = CGPointMake(self.scrollView.frame.size.width, 0);
#pragma mark--- 获取scrollView滑动后选择的日期
//    NSLog(@"%@", self.centerCalendarItem.date);
    self.block(self.centerCalendarItem.date);
}
