...
#define kItemWidth [UIScreen mainScreen].bounds.size.width / 3 - 1
...
//    self = [super initWithFrame:CGRectMake(0, STATUS_AND_NAVIGATION_HEIGHT + 40, frame.size.width, frame.size.height - STATUS_AND_NAVIGATION_HEIGHT - 40)];
    self = [super init];
    if (self) {
        self.view.frame = CGRectMake(0, 0 + 40, frame.size.width, frame.size.height - 40);
        self.view.backgroundColor = [self colorWithRGB:0x000000 alpha:0.3];
        _rootView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight / 1.5)];
        UIView *bottomView = [[UIView alloc] initWithFrame:CGRectMake(0, frame.size.height / 1.5, frame.size.width, frame.size.height / 1.5 - STATUS_AND_NAVIGATION_HEIGHT - 40)];
...
- (void)initFirstTableView {
    _firstTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kItemWidth + 1, kScreenHeight / 1.5) style:UITableViewStylePlain];
...
- (void)initSecondTableView {
    _secondTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxX(_firstTableView.frame), kScreenWidth - kItemWidth - 1, CGRectGetHeight(_firstTableView.frame)) style:UITableViewStylePlain];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if ([tableView isEqual:_firstTableView]) {
        ...
        if (_rightArray.count > firstSelectedIndex) {
            NSArray *array = [_rightArray[firstSelectedIndex] countryArray];
            return array.count;
        } else {
            return 0;
        }
    ...
    if ([tableView isEqual:_firstTableView]) {
        switch (indexPath.section) {
            case 0:
            {
                ...
                if (_leftArray.count > 0) {
                    ...
                    cell.textLabel.text = [_leftArray[indexPath.row] cn_name];
                }
                break;
            }
            default:
        ...
        switch (indexPath.section) {
            case 0:
            {
                ...
                CountryModel *model = array[indexPath.row];
                cell.textLabel.font = [UIFont systemFontOfSize:13];
...
- (void)showSingleOrDouble {
    if (_leftArray.count <= 0) {
        ...
        _secondTableView.frame = CGRectMake( 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT / 1.5);
    } else {
        [_firstTableView setHidden:NO];
        _secondTableView.frame = CGRectMake(kItemWidth + 1, 0, SCREEN_WIDTH - kItemWidth - 1, SCREEN_HEIGHT / 1.5);
    }
}
