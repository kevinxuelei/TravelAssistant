...
- (UIButton *)makeButton:(NSString *) title andIndex:(int)index{
    ...
    float padding = (width - (_image.size.width + [title sizeWithAttributes:@{@"font":[UIFont systemFontOfSize:13.0f]}].width)) / 2;
    ...
    [button setImageEdgeInsets:UIEdgeInsetsMake(11, [title sizeWithAttributes:@{@"font":[UIFont systemFontOfSize:13.0f]}].width + padding + 5, 11, 0)];
...
- (void)setDropdownButtonTitleWithString:(NSString *)titleString
{
    if ([titleString isEqualToString:@""] || titleString == nil) {
        return;
    }
    NSArray *array = @[titleString];
    [self addButtonToView:array];
}
